...
Related: `AGENTS.md` (project + verification rules), `.omo/plans/design-system-foundation.md` (design-system extraction history).
...
| **Primitives (`ui`)** | `src/components/ui/**` | Domain-agnostic building blocks: `Button`, `ButtonGroup`, `Card`, `SectionHeading`, `ProgressBar`, `EmptyState`, `Badge`, `SkillChip`, `Select`, `Collapse`, … | Reused across all domains |
| **Domain / feature** | `src/components/portfolio/**`, `src/components/resume/**`, `src/components/print/**`, `src/components/analytics/**` | Composed UI owned by one product area (home, navigation, layout, project-detail, resume, print) | Reused within that area |
| **Route / app** | `src/app/**` route files and co-located `_components/` (e.g. `src/app/a/_components/`, `src/app/(portfolio)/_components/`) | Page-specific composition, data wiring, route shells | Not reused elsewhere |
...
- **Exports**: keep a folder internally consistent. `src/components/ui/*` uses `default export`; `src/app/**/_components/*` uses `named exports`. Do not mix within the same folder.
...
The import direction is currently enforced by **convention and code review**. If drift appears, add a restricted-import lint rule (or a lightweight check script) to fail cross-layer imports, but do not introduce a heavier framework for it.
