  ...
  const copy = {
    ...
    ko: {
      ...
      surfaces: [
        ...
        {
          label: 'macOS menu bar',
          detail: 'MenuBarExtra가 30초 상태 엔진에서 계산한 현재 일정과 남은 시간을 표시합니다.',
        },
        {
          label: 'Focus automation',
          detail:
        ...
        {
          label: 'Timeline layout',
          detail: '겹치는 시간 블록을 lane으로 배치하고 텍스트 스냅샷 fixture로 회귀를 확인합니다.',
        },
        {
          label: 'CloudKit sync',
          detail:
...
<section class="day-planner-evidence" aria-label={labels.surfacesTitle}>
  ...
  <div class="surface-map">
    ...
    <ol class="surface-list">
      {#each labels.surfaces as item, index (item.label)}
        <li>
  ...
  <div class="verification-card">
    ...
    <dl>
      {#each labels.evidence as item (item.label)}
        <div>
          <dt>{item.label}</dt>
          <dd>
...
<style>
  ...
  .verification-card {
    ...
    box-shadow: 0 12px 30px color-mix(in srgb, var(--color-shadow) 70%, transparent);
  }
