    ...
    },
    {
      ...
      achievements: [
        ...
        },
        {
          ...
          detail:
            '`pnpm test` passes **15 test files / 216 tests**. That includes 127 shared-package tests for weather decisions, outfit recommendation, and notification rules, 49 mobile tests for location and notification storage, and 40 web tests for storage and weather-scene logic. `pnpm typecheck` also passes across web, mobile, and shared packages.',
        },
        {
          tag: 'Data',
    ...
    },
    {
      ...
      achievements: [
        ...
        },
        {
          ...
          detail:
            '한국 공공 API(기상청, 에어코리아)는 비한국 IP의 요청을 차단합니다. Vercel 서버리스 실행 환경을 **icn1(서울)**로 고정하여 인프라 레벨에서 해결했습니다. API 앞단에는 **Upstash Redis** TTL 캐시를 두어 같은 좌표·시간대의 반복 요청이 매번 공공 API로 전달되지 않게 설계했습니다. 현재는 아키텍처 설계와 테스트 데이터를 바탕으로 검증했으며, 실제 호출 절감률은 실 서비스 운영 과정에서 누적되는 트래픽 데이터를 기반으로 측정할 예정입니다.',
        },
        {
          tag: 'Platform',
        ...
        },
        {
          ...
          detail:
            '`pnpm test`는 **15개 테스트 파일 / 216개 테스트**를 통과했습니다. 범위는 `packages/shared`의 날씨 판단·복장 추천·알림 조건 로직 127개, `apps/mobile`의 위치/알림 저장소 49개, `apps/web`의 저장소와 날씨 scene 로직 40개입니다. `pnpm typecheck`도 web/mobile/shared 세 패키지에서 통과했습니다.',
        },
        {
          tag: 'Data',
