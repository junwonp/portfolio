...
Rather than a simple showcase of tool proficiency, this workflow is designed to **control AI's uncertainty (such as hallucinations and unpredictability) through a systematic verification pipeline**, allowing reclaimed engineering resources to be redirected toward high-value domains like high-performance optimization.

### Core Concepts
- **Engineering Control:** Architecture design, security boundaries, product tradeoffs, and final diff reviews remain 100% human-owned and verified.
- **Safety Guardrails:** AI-generated drafts are strictly validated by connecting them to static analysis (`tsc`), linting (`eslint`), automated testing (`vitest`), and actual browser UI inspections before deployment.
- **Context Budgeting:** To maximize the efficiency of LLM's limited context window, global rules and project-specific guides (`AGENTS.md`) are decoupled, enforcing focused and highly accurate iterations.
- **Value Redirection:** Mechanical coding and repetitive tasks are minimized, freeing up engineering resources to solve high-impact problems such as scroll performance optimization and safe database migration.
