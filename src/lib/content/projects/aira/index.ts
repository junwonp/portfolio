  ...
  content: {
    ...
    ko: {
      ...
      summaryDetails: [
        ...
        '**[스트리밍 채팅 및 Optimistic UI]** 스트림 청크를 **Zod**로 실시간 검증하고, TanStack Query **InfiniteQuery** 캐시를 직접 패치하여 **끊김 없는 타이핑 애니메이션**을 구현했습니다. `onMutate`를 통한 **Optimistic Update**로 즉각적인 피드백을 제공했습니다.',
        '**[Type-Safe 캐시 파사드]** QueryClient를 직접 다루는 대신 도메인별로 구조화된 **queryData 파사드**를 설계했습니다. 계층적 쿼리 키 팩토리를 통해 **일괄 무효화 및 Type-Safe한 CRUD** 작업을 수행합니다.',
        ...
        '**[Type-Safe i18n]** **typesafe-i18n**으로 번역 키를 컴파일 타임에 검증합니다. Intl 폴리필 로드 완료 전까지 렌더링을 차단하여 로케일 오작동을 방지하고, 언어 전환 시 OS 설정을 동기화했습니다.',
