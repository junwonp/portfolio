...
/** `src:` and `src="": the lookbehind keeps `mobileSrc`, `srcSet`, and `data-src` out. */
const bodySourcePattern = /(?<![\w$.-])src\s*[:=]\s*(['"])([^'"]+)\1/g;
...
export const bodyImages = (source) => {
  ...
  for (const match of body.matchAll(bodySourcePattern)) {
    if (isPrintableSource(match[2])) images.push(match[2]);
  }
