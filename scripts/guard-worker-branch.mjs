...
if (target === 'production' && branch !== 'main') {
  console.error(
    `Refusing production deploy from ${branch}. Production deploys must come from main.`,
  );
  process.exit(1);
}
...
if (target === 'development' && branch !== 'develop') {
  console.error(
    `Refusing development deploy from ${branch}. Development deploys must come from develop.`,
  );
  process.exit(1);
}
